...
export default function PublishButton({ onOpenSettings }) {
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      setStatus("success");
      setShowResult(true);
      setTimeout(() => { setStatus("idle"); setShowResult(false); }, 8000);
    } catch (e) {
      setError(e.message);
      setStatus("error");
      setShowResult(true);
  ...
  return (
    <div className="relative">
      <button
        ...
        className="flex items-center gap-2 px-4 py-2.5 rounded-xl font-body text-sm transition-all duration-300 hover:scale-[1.02]"
      ...
      >
        ...
        {status === "idle" && (
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="shrink-0">
            {hasToken
              ? <><path d="M12 0C5.37 0 0 5.37 0 12c0 5.31 3.435 9.795 8.205 11.385.6.105.825-.255.825-.57 0-.285-.015-1.23-.015-2.235-3.015.555-3.795-.735-4.035-1.41-.135-.345-.72-1.41-1.23-1.695-.42-.225-1.02-.78-.015-.795.945-.015 1.62.87 1.845 1.23 1.08 1.815 2.805 1.305 3.495.99.105-.78.42-1.305.765-1.605-2.67-.3-5.46-1.335-5.46-5.925 0-1.305.465-2.385 1.23-3.225-.12-.3-.54-1.53.12-3.18 0 0 1.005-.315 3.3 1.23.96-.27 1.98-.405 3-.405s2.04.135 3 .405c2.295-1.56 3.3-1.23 3.3-1.23.66 1.65.24 2.88.12 3.18.765.84 1.23 1.905 1.23 3.225 0 4.605-2.805 5.625-5.475 5.925.435.375.81 1.095.81 2.22 0 1.605-.015 2.895-.015 3.3 0 .315.225.69.825.57A12.02 12.02 0 0024 12c0-6.63-5.37-12-12-12z" fill="currentColor" stroke="none"/></>
        ...
        <span className="font-body text-xs">
          ...
          {status === "success" && "Berhasil dipublish!"}
          {status === "error" && "Gagal publish"}
        </span>
      </button>
        ...
        {showResult && (
          <motion.div
            ...
            style={{
              ...
              border: status === "success" ? "1px solid rgba(74,222,128,0.25)" : "1px solid rgba(239,68,68,0.25)",
          ...
          >
            {status === "success" && result && (
              <div className="space-y-2">
                <div className="flex items-center gap-2 mb-3">
                  <div className="w-7 h-7 rounded-full flex items-center justify-center" style={{ background: "rgba(74,222,128,0.12)", border: "1px solid rgba(74,222,128,0.3)" }}>
                ...
                <p className="font-body text-xs" style={{ color: "rgba(255,255,255,0.5)", lineHeight: "1.7" }}>
                  Commit: <span style={{ color: "rgba(255,255,255,0.75)", fontFamily: "monospace" }}>#{result.commitSha}</span>
                </p>
                <p className="font-body text-xs" style={{ color: "rgba(255,255,255,0.4)", lineHeight: "1.7" }}>
                  Vercel akan auto-rebuild dalam ~30–60 detik. Setelah itu semua orang bisa lihat perubahan.
                </p>
                {result.commitUrl && (
                  <a href={result.commitUrl} target="_blank" rel="noopener noreferrer"
                    className="inline-flex items-center gap-1 font-body text-xs mt-1 transition-opacity hover:opacity-75"
            ...
            {status === "error" && (
              <div>
                ...
                <p className="font-body text-xs" style={{ color: "rgba(255,255,255,0.4)", lineHeight: "1.7" }}>{error}</p>
                <button onClick={onOpenSettings} className="mt-3 font-body text-xs underline" style={{ color: "#e8c4a0" }}>
