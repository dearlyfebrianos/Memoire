/**
 * UserManagement.jsx
 * Menerima credentials via props dari AdminDashboard (dari static import authData.js)
 * Setelah save → update in-memory langsung via onUpdate callback
 * Tidak ada static import dari authData di sini — semua dari props
 */
...
export default function UserManagement({
  ...
  // Sync saat parent mengirim data baru (misal setelah manual refresh)
  useEffect(() => {
  ...
  const handleUpdate = async (oldUsername, newUsername, newPassword, newAvatar, newBio, newDisplayName) => {
    setStatus("saving");
    setError("");
    try {
      ...
      // Update in-memory langsung agar UI reflect perubahan tanpa reload
      setUsers(updatedUsers);
      if (onUpdate) onUpdate(updatedUsers);

      // Ikuti username baru jika berubah
  ...
  // ── Simpan perubahan lockdown code ──────────────────────────────────────────
    ...
    <div className="space-y-8 max-w-5xl mx-auto pt-4">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-white/5 pb-8">
        ...
        <div className="flex items-center gap-3">
          {/* Tombol manual refresh dari GitHub */}
          {onRefresh && (
            <button
              onClick={onRefresh}
      ...
      {activeUser ? (
        <div className="w-full">
          <UserCard
            // Key gabungan semua field — React re-mount setiap kali ada field berubah setelah save
            key={`${activeUser.username}|${activeUser.displayName}|${activeUser.avatar}|${activeUser.bio}|${activeUser.password}`}
            user={activeUser}
            onSave={handleUpdate}
        ...
        <div className="flex items-center justify-center py-20">
          <p className="font-body text-sm text-white/20 uppercase tracking-widest">
            Tidak ada data user.
          </p>
        </div>
      )}

      {/* Lockdown Code — Owner Only */}
      {isOwner && (
        <LockdownManager
...
function UserCard({ user, onSave, isSelf, status }) {
  ...
  // Karena key sudah berubah saat parent update, component ini selalu fresh.
  // useEffect ini sebagai safety net tambahan.
  useEffect(() => {
