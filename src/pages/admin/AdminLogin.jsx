...
import { fetchLiveAuthData, loginWithCredentials, isLoggedIn } from "../../data/auth";
...
export default function AdminLogin() {
  ...
  const [loadingMsg, setLoadingMsg] = useState("Masuk...");
  ...
  const doLogin = useCallback(async () => {
    ...
    setLoading(true);
    setError("");
    setLoadingMsg("Memverifikasi...");

    try {
      // 1. Selalu fetch kredensial terbaru dari GitHub
      setLoadingMsg("Memuat kredensial...");
      const liveData = await fetchLiveAuthData();

      let credentials = liveData?.credentials;

      // Fallback ke static import jika GitHub tidak bisa diakses
      if (!credentials) {
        try {
          const mod = await import("../../data/authData");
      ...
      // Sedikit delay agar terasa natural
      await new Promise((r) => setTimeout(r, 400));

      // 2. Login dengan kredensial live
      const session = loginWithCredentials(username.trim(), password, credentials);
      if (session) {
      ...
      >
        ...
        >
          ...
          <div className="text-center mb-8">
            ...
            <h1 className="font-display text-2xl mb-1" style={{ color: "rgba(255,255,255,0.9)", fontWeight: 300 }}>
              Admin Panel
            </h1>
            <p className="font-body text-xs" style={{ color: "rgba(255,255,255,0.35)", letterSpacing: "0.1em" }}>
              MEMOIRE
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label
                className="block font-body text-xs mb-2 uppercase"
