...
const CFG = {
  info:    { bg:'rgba(59,130,246,.12)', border:'rgba(59,130,246,.4)', text:'#93c5fd', icon:'💡' },
  success: { bg:'rgba(34,197,94,.1)',   border:'rgba(34,197,94,.4)',  text:'#86efac', icon:'✅' },
  warning: { bg:'rgba(245,158,11,.1)',  border:'rgba(245,158,11,.4)', text:'#fcd34d', icon:'⚠️' },
  error:   { bg:'rgba(220,38,38,.1)',   border:'rgba(220,38,38,.4)',  text:'#fca5a5', icon:'🚨' },
};
...
export default function AnnounceModal() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const r = await fetch('/announce.json?_=' + Date.now(), { cache: 'no-store' });
        if (!r.ok) return;
        const d = await r.json();
        if (!mounted || !d?.active || !d?.message) return;
        if (localStorage.getItem(KEY) === d.updatedAt) return;
        setAnn(d); setOpen(true); setTimer(SECS);
      } catch {}
    };
    load();
  ...
  return (
    <>
      ...
      <div style={{
        ...
        zIndex:99999, width:'min(420px,calc(100vw - 32px))',
        ...
        <div style={{ display:'flex', alignItems:'center', gap:12,
          padding:'16px 20px 12px', borderBottom:'1px solid rgba(255,255,255,.07)' }}>
          ...
          <div style={{ flex:1 }}>
            <p style={{ margin:0, fontSize:11, fontWeight:700, letterSpacing:'.08em',
              textTransform:'uppercase', color:c.text, opacity:.7 }}>Pengumuman</p>
            <p style={{ margin:'2px 0 0', fontSize:13, fontWeight:600, color:c.text, textTransform:'capitalize' }}>
              {ann.type}
            </p>
          </div>
          <button onClick={close_} aria-label="Close" style={{
            background:'rgba(255,255,255,.07)', border:'1px solid rgba(255,255,255,.12)',
            borderRadius:8, width:32, height:32, cursor:'pointer',
            display:'flex', alignItems:'center', justifyContent:'center',
            color:'rgba(255,255,255,.5)', fontSize:20, lineHeight:1,
          }}>×</button>
        </div>

        {/* Body */}
        <div style={{ padding:'16px 20px' }}>
          <p style={{ margin:0, fontSize:15, lineHeight:1.65, color:'rgba(255,255,255,.88)' }}>
            {ann.message}
          </p>
        ...
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between',
          padding:'10px 20px 14px', borderTop:'1px solid rgba(255,255,255,.07)' }}>
          <span style={{ fontSize:11, color:'rgba(255,255,255,.3)' }}>Menutup dalam {timer}s</span>
