...
export default function Navbar() {
  ...
  const isChapterActive = chapters.some((c) => location.pathname === `/chapter/${c.slug}`);
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between">
          ...
          <Link to="/" className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
            ...
            <span className="font-display text-lg sm:text-xl" style={{ color: "#e8c4a0", letterSpacing: "0.18em" }}>
              Memoire
            </span>
          </Link>
          ...
          <div className="hidden md:flex items-center gap-0.5">
            ...
            <div ref={dropdownRef} className="relative">
              <button
                ...
                style={{ color: isChapterActive || chapterOpen ? "#e8c4a0" : "rgba(255,255,255,0.6)", letterSpacing: "0.05em" }}
              >
                Chapter
                <motion.svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"
                  animate={{ rotate: chapterOpen ? 180 : 0 }} transition={{ duration: 0.22 }}>
              ...
              <AnimatePresence>
                {chapterOpen && (
                  <motion.div
                    ...
                    style={{
                      /* right-align so it never overflows right edge */
                      right: 0,
                      minWidth: "220px",
                      ...
                      boxShadow: "0 20px 50px rgba(0,0,0,0.6), inset 0 1px 0 rgba(255,255,255,0.07)",
                    }}
                  >
                    <div className="p-2">
                      {chapters.length === 0 ? (
                        <p className="px-4 py-3 text-xs font-body" style={{ color: "rgba(255,255,255,0.35)" }}>
                          Belum ada chapter
                        </p>
                      ) : (
                          ...
                          return (
                            <motion.div key={chapter.id}
                              ...
                              <Link
                                ...
                                style={{
                                  background: active ? `${chapter.accentColor}14` : "transparent",
                                  border: active ? `1px solid ${chapter.accentColor}28` : "1px solid transparent",
                                }}
                                onMouseEnter={(e) => { if (!active) e.currentTarget.style.background = "rgba(255,255,255,0.06)"; }}
                                onMouseLeave={(e) => { if (!active) e.currentTarget.style.background = active ? `${chapter.accentColor}14` : "transparent"; }}
                              >
                                <span className="text-base leading-none flex-shrink-0">{chapter.emoji}</span>
                                <div className="flex-1 min-w-0">
                                  <p className="font-body text-sm truncate"
                                    style={{ color: active ? chapter.accentColor : "rgba(255,255,255,0.78)" }}>
                                    {chapter.label}
                                  </p>
                                  <p className="font-body" style={{ fontSize: "0.63rem", color: "rgba(255,255,255,0.3)" }}>
                                    {chapter.years} · {chapter.photos.length} foto
                                  </p>
                                </div>
                                {active && <div className="w-1.5 h-1.5 rounded-full flex-shrink-0" style={{ background: chapter.accentColor }} />}
                              </Link>
                            </motion.div>
                          );
                        })
                    ...
                    <div style={{ borderTop: "1px solid rgba(255,255,255,0.06)" }}>
                      <Link to="/gallery" onClick={() => setChapterOpen(false)}
          ...
          >
            {[0, 1, 2].map((i) => (
              <span key={i} className="block w-4 h-px transition-all duration-300"
                style={{
                  background: "#e8c4a0",
          ...
          <>
            ...
            >
              ...
              <div className="flex items-center justify-between px-5 py-4" style={{ borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
                <span className="font-display text-base" style={{ color: "#e8c4a0", letterSpacing: "0.15em" }}>Menu</span>
              ...
              <div className="px-3 py-4 space-y-1">
                ...
                {/* Gallery */}
                <MobileNavLink to="/gallery" active={isActive("/gallery")} onClick={() => setMobileOpen(false)}>
                  Gallery
                </MobileNavLink>
                ...
                <div>
                  <button
                    ...
                    style={{
                      color: isChapterActive ? "#e8c4a0" : "rgba(255,255,255,0.7)",
                      background: isChapterActive ? "rgba(232,196,160,0.07)" : "transparent",
                    }}
                  >
                    ...
                    <div className="flex items-center gap-2">
                      {chapters.length > 0 && (
                        <span className="px-2 py-0.5 rounded-full font-body text-xs"
                          style={{ background: "rgba(255,255,255,0.08)", color: "rgba(255,255,255,0.4)", fontSize: "0.65rem" }}>
                          {chapters.length}
                        </span>
                      )}
                      <motion.svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"
                        animate={{ rotate: mobileChapterOpen ? 180 : 0 }} transition={{ duration: 0.22 }}>
                        <polyline points="6 9 12 15 18 9"/>
                      </motion.svg>
                    </div>
                  </button>
                          ...
                          {chapters.map((chapter) => {
                            ...
                            return (
                              <Link
                                ...
                                style={{
                                  background: active ? `${chapter.accentColor}14` : "transparent",
                                  border: active ? `1px solid ${chapter.accentColor}22` : "1px solid transparent",
                                }}
                              >
                                <span className="text-sm flex-shrink-0">{chapter.emoji}</span>
                                <div className="flex-1 min-w-0">
                                  <p className="font-body text-sm truncate"
                                    style={{ color: active ? chapter.accentColor : "rgba(255,255,255,0.72)" }}>
                                    {chapter.label}
                                  </p>
                                  <p className="font-body" style={{ fontSize: "0.62rem", color: "rgba(255,255,255,0.3)" }}>
                                    {chapter.years} · {chapter.photos.length} foto
                                  </p>
                                </div>
                              </Link>
                ...
                {/* About */}
                <MobileNavLink to="/about" active={isActive("/about")} onClick={() => setMobileOpen(false)}>
                  About
                </MobileNavLink>
              ...
              <div className="absolute bottom-6 left-0 right-0 flex justify-center">
                <p className="font-display text-xs" style={{ color: "rgba(255,255,255,0.12)", letterSpacing: "0.2em" }}>
                  MEMOIRE
                </p>
              </div>
            </motion.div>
...
function NavLink({ to, active, children }) {
  return (
    <Link to={to}
      ...
      style={{ color: active ? "#e8c4a0" : "rgba(255,255,255,0.6)", letterSpacing: "0.05em" }}>
      {children}
      {active && (
        <motion.span layoutId="nav-dot"
          className="absolute bottom-0.5 left-1/2 -translate-x-1/2 w-1 h-1 rounded-full"
